...
public class PMemoryConfig
{
    ...
    @Config("pmemory.splits-per-node")
    public PMemoryConfig setSplitsPerNode(int splitsPerNode)
    {
    ...
    @Mandatory(name = "pmemory.max-data-per-node",
            description = "Define memory limit for pages stored in this connector per each node",
            defaultValue = "128MB",
            required = true)
    @Config("pmemory.max-data-per-node")
    public PMemoryConfig setMaxDataPerNode(DataSize maxDataPerNode)
    {
