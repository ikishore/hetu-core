...
import io.prestosql.plugin.memory.statistics.TpcdsTableStatisticsFactory;
import io.prestosql.spi.statistics.TableStatistics;
//import io.prestosql.plugin.tpcds.statistics.TpcdsStatisticsRecorder;
        ...
        implements ConnectorMetadata
{
    ...
    public synchronized ConnectorTableHandle getTableHandle(ConnectorSession session, SchemaTableName schemaTableName)
    {
        ...
        return new MemoryTableHandle(id, schemaTableName.getTableName());
    }
    ...
    public TableStatistics getTableStatistics(ConnectorSession session, ConnectorTableHandle tableHandle, Constraint constraint)
    {
        ...
        //System.out.println("Memory Metadata getTableStatistics");
        MemoryTableHandle memoryTableHandle = (MemoryTableHandle) tableHandle;

        TableStatistics stats = TableStatistics.empty();

        try {
            Table table = Table.getTable(memoryTableHandle.getTableName());
            System.out.println("Memory Metadata getTableStatistics: "+table.getName());
            String schemaName = "tpcds" ; //scaleFactorSchemaName(memoryTableHandle.getScaleFactor());
            stats = tpcdsTableStatisticsFactory.create(schemaName, table, getColumnHandles(session, tableHandle));
        } catch (Exception e) {
    ...
    public Optional<LimitApplicationResult<ConnectorTableHandle>> applyLimit(ConnectorSession session, ConnectorTableHandle handle, long limit)
    {
        ...
        return Optional.of(new LimitApplicationResult<>(
                new MemoryTableHandle(table.getId(), table.getTableName(), OptionalLong.of(limit), OptionalDouble.empty(), table.getPredicate()),
                true));
    }
    ...
    public Optional<ConnectorTableHandle> applySample(ConnectorSession session, ConnectorTableHandle handle, SampleType sampleType, double sampleRatio)
    {
        ...
        return Optional.of(new MemoryTableHandle(table.getId(), table.getTableName(), table.getLimit(), OptionalDouble.of(table.getSampleRatio().orElse(1) * sampleRatio), table.getPredicate()));
    }
}
