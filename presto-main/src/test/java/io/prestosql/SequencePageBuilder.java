...
public final class SequencePageBuilder
{
    ...
    public static Page createSequencePageShared(List<? extends Type> types, int length, Random rnd, int... initialValues)
    {
        Block[] blocks = new Block[initialValues.length];
        for (int i = 0; i < blocks.length; i++) {
            Type type = types.get(i);
            ...
            if (i < blocks.length - 1) {
                ...
                blocks[i] = BlockAssertions.createQuerySetBlock(initialValue, rnd, length);
            }
        }
    ...
    public static Page createSequenceUniformPageShared(List<? extends Type> types, int length, int initial, Random rnd, Long range, int numberOfQueries, boolean randomSet)
    {
        ...
        for (int i = 0; i < blocks.length; i++) {
            ...
            if (i == 0) {
                if (type.equals(BIGINT)) {
                    blocks[i] = BlockAssertions.createLongSequenceBlock(initial, initial + length);
                }
                else {
                    throw new IllegalStateException("Unsupported type " + type);
                }
                ...
                if (type.equals(BIGINT)) {
                    blocks[i] = BlockAssertions.createLongUniformBlock(rnd, range, length);
                }
                else {
                    throw new IllegalStateException("Unsupported type " + type);
                }
                ...
                if (randomSet) {
                    blocks[i] = BlockAssertions.createQuerySetBlock(numberOfQueries, rnd, length);
                }
                else {
                    blocks[i] = BlockAssertions.createUniformQuerySetBlock(numberOfQueries, rnd, length);
                }
            }
        }
    ...
    public static Page createSequenceProbabilisticPageShared(List<? extends Type> types, int length, int initial, Random rnd, int numberOfQueries, double probability)
    {
        Block[] blocks = new Block[types.size()];
        for (int i = 0; i < blocks.length; i++) {
            Type type = types.get(i);
            ...
            if (i == 0) {
                ...
                blocks[i] = BlockAssertions.createProbabilisticQuerySetBlock(numberOfQueries, rnd, probability, length);
            }
        }
    ...
    public static Page createUniformPageShared(List<? extends Type> types, int length, Random rnd, Long range, int numberOfQueries, boolean randomSet)
    {
        Block[] blocks = new Block[types.size()];
        for (int i = 0; i < blocks.length; i++) {
            Type type = types.get(i);
