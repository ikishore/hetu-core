...
{
    private static final Logger LOG = Logger.get(RecoveryManager.class);
    ...
    public RecoveryManager(@Nonnull SnapshotUtils snapshotUtils, Session session, QueryId queryId)
    {
        ...
        this.retryCount = 0;
        this.querySnapshotManager = snapshotUtils.getOrCreateQuerySnapshotManager(queryId, session);
    ...
    {
        if (retryTimer.isPresent()) {
            LOG.warn("Query %s finished after resume, but recovery was not done", queryId.getId());
            return true;
        }
        return false;
    }
